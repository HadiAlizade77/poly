// ─── Shared Liquidity Quality Scorer ─────────────────────────────────────────
// Generic spread + depth analysis, works for all categories.
// 0 = terrible liquidity, 100 = excellent.
    ...
    score(context: ScorerInput): ScorerDimension {
      ...
      if (spread !== null) {
        if (spread <= params.tight_spread) {
          spreadScore = Math.round(80 + (1 - spread / params.tight_spread) * 20);
        } else if (spread >= params.wide_spread) {
          const excess = Math.min(2, (spread - params.wide_spread) / params.wide_spread);
          spreadScore = Math.round(20 - excess * 10);
        } else {
          const pos = 1 - (spread - params.tight_spread) / (params.wide_spread - params.tight_spread);
          spreadScore = Math.round(20 + pos * 60);
        }
      }
      ...
      if (liquidity !== null) {
        if (liquidity >= params.good_liquidity) {
          liqScore = Math.round(80 + Math.min(20, ((liquidity / params.good_liquidity) - 1) * 5));
        } else if (liquidity <= params.min_liquidity) {
          liqScore = Math.round((liquidity / params.min_liquidity) * 20);
        } else {
          const pos = (liquidity - params.min_liquidity) / (params.good_liquidity - params.min_liquidity);
          liqScore = Math.round(20 + pos * 60);
        }
      }
      ...
      return {
        ...
        metadata: { spread, spread_score: spreadScore, liquidity, liquidity_score: liqScore, depth_score: depthScore },
      };
    },
...
export function createLiquidityQualityScorer(category: string): ContextScorer {
  return makeScorer(category);
}

/** Pre-built instances for common categories. */
export const politicsLiquidityScorer = makeScorer('politics');
export const sportsLiquidityScorer = makeScorer('sports');
export const eventsLiquidityScorer = makeScorer('events');
